...
def main(argv):
  ...
  ## adjacency, features, labels, label_indices = sbm.create_sbm(50,0.6,0.4,4)

  ## Lets first visualize the graph
  ...
  ## check = adjacency.toarray()
  ## print(check[0][0])

  adjacency = sparsity.add_sparsity(adjacency , 0.9)
  ...
  ## This is the graph after the community detction
  ## visualize.draw_graph(adjacency , clusters.tolist())

  # Prints some metrics used in the paper.

  ## adjacency._set_arrayXarray(0, 0, 1)
  ...
  for cluster_id in np.unique(clusters):
  	lists = np.where(clusters == cluster_id)[0]
  	print("The size of the cluster ",cluster_id," Is ",lists.shape)

  print("Now we will try to run the Spectral Clustering")

## At first i will present the Spectral clustering by using scikit
## adjacency - This is the adjacency matrix that will be used





  ## Below is the implementation of spectral Clustering by me
  """
  D = np.diag(np.sum(np.array(adjacency.todense()), axis=1))
  ...
  U = np.array( v[:, i[1]] )
  clustering = KMeans(init='k-means++', n_clusters=15)
  clustering.fit(U)
  """
  ad = adjacency.toarray();
  clustering = SpectralClustering(n_clusters=16,assign_labels='discretize',random_state=0).fit(ad)
  labels_spec = clustering.labels_

  ## visualize.draw_graph(adjacency , labels_spec)
  
  print(type(labels_spec) , labels_spec.shape)

  ## Now we will print the metric for the Spectral Clustering
  print("Now we will find the metric for spectral clustering")

  print('Conductance:', metrics.conductance(adjacency, labels_spec))
  print('Modularity:', metrics.modularity(adjacency, labels_spec))
  print(
      'NMI:',
      sklearn.metrics.normalized_mutual_info_score(
          labels, labels_spec[label_indices], average_method='arithmetic'))
  precision = metrics.pairwise_precision(labels, labels_spec[label_indices])
  recall = metrics.pairwise_recall(labels, labels_spec[label_indices])
  print('F1:', 2 * precision * recall / (precision + recall))

  for cluster_id in np.unique(labels_spec):
  	lists = np.where(labels_spec == cluster_id)[0]
  	print("The size of the cluster ",cluster_id," Is ",lists.shape)
